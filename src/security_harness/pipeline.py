...
class HandlerProtocol(Protocol):
    name: str
    async def execute(self, event: SecurityEvent) -> tuple[HandlerResult, list[RiskSignal]]: ...
...
class Pipeline:
    def __init__(self, handler: HandlerProtocol | None = None, handler_timeout_ms: int = 1000): self.handler_timeout_ms = handler_timeout_ms; self.handler = handler or MockRuleHandler()
    @property
    def handler_name(self) -> str: return self.handler.name
    async def execute(self, event: SecurityEvent) -> PipelineOutput:
        try:
            result, signals = await asyncio.wait_for(self.handler.execute(event), self.handler_timeout_ms / 1000)
            return PipelineOutput(result.decision or Decision.REVIEW, signals, [result])
